...
//Filtra los eventos con fecha pasada.
const pastEvents = events.filter(evento => evento.date < currentDate);
...
//Inicializa la página con los eventos pasados.
...
let filters = {
    'searchStr': '',
    'selectedCategories': [],
    'filteredEvents': pastEvents //Por defecto contiene los eventos pasados.
};
...
const applyFilters = () => {
    if (selectedCategories.length > 0) {
        const res = pastEvents.filter(evento => selectedCategories.includes(evento.category))
        filteredEvents = res;
    } else {
